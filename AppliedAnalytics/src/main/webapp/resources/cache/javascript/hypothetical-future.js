...
function updateHypotheticalWidget(id) {
	...
	//.on("mouseover", function (d) { showTip(this, d.y); });
	...
	var dragRect = newg.append("rect").attr("id", "drag_line").attr("x",
			function(d) {
				return d.x;
			}).attr("y", function(d) {
		return d.y;
	}).attr("height", smallH).attr("width", 2).attr("fill", "grey").attr(
			"cursor", "move") // can also do cursor
	.call(drag);
	...
	function dragEvent(d) {
		...
		dragRect.attr("x", d.x = newX).attr("y", d.y = d.y);
	}

	/***********************
	TODO:add noise to future
	CHECKBOX OPTION
	 ************************/
	function addNoise() {
	}
	...
	function appendCircle() {
		...
		trend_g
				...
				.on(
						...
						function() {
							...
							var length_at_point = 0, total_length = line_path
									.getTotalLength(), mouse = d3
									.mouse(line_path);

							// find point on the line to draw tangent on, 
							var INTERVAL = 100;
							while (line_path.getPointAtLength(length_at_point).x < mouse[0]
							...
							var point = line_path
									.getPointAtLength(length_at_point), prev = {}, next = {}, delta = {};

							if (length_at_point > 1
									&& length_at_point < (total_length - 1)) {
								prev = line_path
										.getPointAtLength(length_at_point - 1);
								next = line_path
										.getPointAtLength(length_at_point + 1);
								delta = {
									x : next.x - prev.x,
							...
							trend_g.append("svg:text").attr('id', 'text').attr(
									'x', point.x).attr('y', point.y).attr('dy',
									'.31em').text('test');

							var tangent = svg.append('circle').attr('id',
									'tangent').attr('r', 3).attr('cx', point.x)
									.attr('cy', point.y);
		...
		/* TODO: FIX - get circle to show up on historical path */
		/* HYPOTHETICAL LINE */
		hypo_g
				.append('path')
				.classed('overlay', true)
				...
				.on(
						...
						function() {
							...
							var length_at_point2 = 0, total_length2 = line_path2
									.getTotalLength(), mouse = d3
									.mouse(line_path2);
							...
							var INTERVAL2 = 100;
							while (line_path2
									.getPointAtLength(length_at_point2).x < mouse[0]
									&& length_at_point2 < total_length2)
							...
							var point2 = line_path2
									.getPointAtLength(length_at_point2), prev2 = {}, next2 = {}, delta2 = {};

							if (length_at_point2 > 1
									&& length_at_point2 < (total_length2 - 1)) {
								prev2 = line_path2
										.getPointAtLength(length_at_point2 - 1);
								next2 = line_path2
										.getPointAtLength(length_at_point2 + 1);
								delta2 = {
									x2 : next2.x - prev2.x,
							...
							var tangent2 = svg.append('circle').attr('id',
									'tangent2').attr('r', 3).attr('cx',
									point2.x).attr('cy', point2.y);
						}).on('mouseout', function() {
	...
	/***********************
	 BUTTONS: smoothed, normalized, raw
	 ************************/
	var smoothedButton;
	...
	function updateSmoothed() {
		...
		svg.append("path").attr("id", "smoothID").attr("class", "line").attr(
				"d", smoothedLine(hypoData)).style("stroke-dasharray", ("3,3"))
				.style({
					'fill' : 'none'
				}).style({
					'stroke' : '#1f77b4'
		...
		svg.append("path").attr("id", "smoothID2").attr("class", "line").attr(
				"d", smoothedTrend(trendData)).style({
			'fill' : 'none'
		}).style({
			'stroke' : '#1f77b4'
		}).style({
	...
	function updateRaw() {
		...
		var smoothedTrend = d3.svg.line().x(function(d, i) {
			return x(i);
		}).y(function(d) {
			return y(d);
		});
